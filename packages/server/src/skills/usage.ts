...
function getUsagePath(skillName: string): string {
  const safeName = skillName.replace(/[^a-z0-9-_]/gi, '-').toLowerCase()
  return join(getUsageDir(), `${safeName}.json`)
}

export function recordSkillFire(entry: SkillUsageEntry): void {
  const path = getUsagePath(entry.skillName)
  const entries: SkillUsageEntry[] = existsSync(path)
...
export function getSkillUsageStats(skillName: string): SkillUsageStats {
  ...
  const entries: SkillUsageEntry[] = JSON.parse(readFileSync(path, 'utf-8'))
  const firesByDay: Record<string, number> = {}
  ...
  for (const e of entries) {
    ...
    if (e.channel) channels[e.channel] = (channels[e.channel] ?? 0) + 1
    if (e.timeSavedSeconds) totalTimeSaved += e.timeSavedSeconds
  }
...
export function getAllUsageStats(): SkillUsageStats[] {

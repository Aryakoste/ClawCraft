...
export async function generateRoutes(app) {
    ...
    app.post('/api/generate', async (req, reply) => {
        const { description, triggerType, permissions, channels, outputFormat, schedule } = req.body;
...
Output format: ${outputFormat}`;
    ...
    app.post('/api/generate/chat', async (req, reply) => {
        ...
        try {
            ...
            if (!reply.raw.destroyed) {
                reply.raw.write(`data: ${JSON.stringify({ error: e.message, done: true })}\n\n`);
            }
        }
        if (!reply.raw.destroyed)
            reply.raw.end();
    ...
    app.post('/api/generate/day', async (req, reply) => {

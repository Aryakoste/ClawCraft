...
import { writeSkill, writeSkillToPath, deleteSkill, duplicateSkill, skillToZip, getSkillHistory, restoreSkillVersion, } from '../skills/writer.js';
import { readFileSync, existsSync } from 'fs';
import { scanSkillContent, detectConflicts } from '../scanner/scanner.js';
export async function skillsRoutes(app) {
    // List all skills
    ...
    app.put('/api/skills/update', async (req, reply) => {
    ...
    // Duplicate a skill
    app.post('/api/skills/duplicate', async (req, reply) => {
    ...
    // Download as ZIP
    app.get('/api/skills/:name/download', async (req, reply) => {
        const skills = readAllSkills();
        const skill = skills.find(s => s.name === req.params.name);
        if (!skill)
            return reply.code(404).send({ error: 'Skill not found' });
        const buf = skillToZip(skill.name, skill.raw);
        reply.header('Content-Disposition', `attachment; filename="${skill.name}.zip"`);
        reply.header('Content-Type', 'application/zip');
        return reply.send(buf);
    });
    // Get skill version history
    app.get('/api/skills/:name/history', async (req) => {
        return getSkillHistory(req.params.name);
    });
    // Get a specific history version's content
    app.get('/api/skills/:name/history/content', async (req, reply) => {
    ...
    app.post('/api/skills/restore', async (req, reply) => {
    ...
    // Scan a skill
    app.get('/api/skills/:name/scan', async (req, reply) => {
        const skills = readAllSkills();
        const skill = skills.find(s => s.name === req.params.name);
        if (!skill)
            return reply.code(404).send({ error: 'Skill not found' });

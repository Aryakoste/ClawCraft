import { writeFileSync, mkdirSync, existsSync, copyFileSync, unlinkSync, readdirSync, } from 'fs';
...
export function writeSkill(skillName, content) {
...
export function writeSkillToPath(filePath, content) {
...
export function duplicateSkill(sourceName, targetName) {
    const sourcePath = getSkillPath(sourceName);
    if (!existsSync(sourcePath))
        throw new Error(`Skill not found: ${sourceName}`);
    const source = readSkill(sourcePath);
    const newContent = source.raw.replace(/^name:\s*.+$/m, `name: "${targetName.replace(/[^a-z0-9-_]/gi, '-').toLowerCase()}"`);
    return writeSkill(targetName, newContent);
}
export function skillToZip(skillName, content) {
...
// History management
function backupSkillVersion(skillName, filePath) {
    try {
...
export function getSkillHistory(skillName) {
    const historyDir = getHistoryDir();
    const safeName = skillName.replace(/[^a-z0-9-_]/gi, '-').toLowerCase();
    const skillHistoryDir = join(historyDir, safeName);
    if (!existsSync(skillHistoryDir))
        return [];
    try {
        return readdirSync(skillHistoryDir)
            ...
            label: f.replace('.md', '').replace(/T/g, ' ').replace(/-/g, (m, i) => i > 9 ? ':' : '-'),
        }));
    }
    catch {
        return [];
...
export function restoreSkillVersion(skillName, versionPath) {
